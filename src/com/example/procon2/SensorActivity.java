...
public class SensorActivity implements SensorEventListener {
	...
	public SensorActivity(Context context) {
		...
		mAccelerometer = sensorManager
				.getDefaultSensor(Sensor.TYPE_ACCELEROMETER); // 加速度センサー
		mMagneticField = sensorManager
				.getDefaultSensor(Sensor.TYPE_MAGNETIC_FIELD); // 磁気センサー
	...
	public void onSensorChanged(SensorEvent event) {
		if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
			...
			/** 　移動しているかどうかの判定アルゴリズム　 **/
			// ローパスフィルタで重力値を抽出　Isolate the force of gravity with the low-pass
			// filter.
		...
		if (mAccelerometerValues != null && mMagneticValues != null) {
			...
			// 傾きの取得
			SensorManager
					.getOrientation(remapRotaionMatrixR, orientationValues);
		}
